...
void OLED_SPI_Init(void) {
    ...
    OLED_SPI_W_CMD(0xAE);//--turn off oled panel
    OLED_SPI_W_CMD(0x00);//---set low column address
    OLED_SPI_W_CMD(0x10);//---set high column address
    OLED_SPI_W_CMD(0x40);//--set start line address  Set Mapping RAM Display Start Line (0x00~0x3F)
    OLED_SPI_W_CMD(0x81);//--set contrast control register
    OLED_SPI_W_CMD(0x00); // 亮度变化量很小
    OLED_SPI_W_CMD(0xA1);//--Set SEG/Column Mapping     0xa0左右反置 0xa1正常
    OLED_SPI_W_CMD(0xC8);//Set COM/Row Scan Direction   0xc0上下反置 0xc8正常
    OLED_SPI_W_CMD(0xA6);//--set normal display
    OLED_SPI_W_CMD(0xA8);//--set multiplex ratio(1 to 64)
    OLED_SPI_W_CMD(0x3f);//--1/64 duty
    OLED_SPI_W_CMD(0xD3);//-set display offset	Shift Mapping RAM Counter (0x00~0x3F)
    OLED_SPI_W_CMD(0x00);//-not offset
    OLED_SPI_W_CMD(0xD5);//--set display clock divide ratio/oscillator frequency
    OLED_SPI_W_CMD(0xC0);//--set divide ratio, Set Clock as 100 Frames/Sec
    OLED_SPI_W_CMD(0xD9);//--set pre-charge period
    OLED_SPI_W_CMD(0xF1);//Set Pre-Charge as 15 Clocks & Discharge as 1 Clock
    OLED_SPI_W_CMD(0xDA);//--set com pins hardware configuration
    OLED_SPI_W_CMD(0x12);
    OLED_SPI_W_CMD(0xDB);//--set vcomh
    OLED_SPI_W_CMD(0x40);//Set VCOM Deselect Level
    OLED_SPI_W_CMD(0x20);//-Set Page Addressing Mode (0x00/0x01/0x02)
    OLED_SPI_W_CMD(0x02);//
    OLED_SPI_W_CMD(0x8D);//--set Charge Pump enable/disable
    OLED_SPI_W_CMD(0x14);//--set(0x10) disable
...
void OLED_Display_On(void) {
    ...
    OLED_SPI_W_CMD(0XAF);  //DISPLAY ON
}
